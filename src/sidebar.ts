...
function getCenterVisibleElement(): HTMLElement | null {
  // Claude.ai selectors first; ChatGPT selectors as fallback
  const messageNodes = Array.from(
    document.querySelectorAll<HTMLElement>(
      '[data-testid="user-message"], div[class*="font-claude-response"], [data-message-author-role="user"], [data-message-author-role="assistant"]'
    )
  );
...
function getSelectedBookmarkTarget(): {
  ...
  // Claude.ai: find the nearest message node directly
  const claudeNode = anchorElement.closest<HTMLElement>(
    '[data-testid="user-message"], div[class*="font-claude-response"]'
  );
  if (claudeNode) return { element: claudeNode, selectedText };

  // ChatGPT fallback
  const roleNode = anchorElement.closest<HTMLElement>(
    '[data-message-author-role="user"], [data-message-author-role="assistant"]'
  );
  if (!roleNode) return null;

  return {
    element: getMessageContainer(roleNode),
    selectedText
  };
}
